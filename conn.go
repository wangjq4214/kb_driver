...
import (
	...
	"github.com/wangjq4214/kb_driver/oid"
	"github.com/wangjq4214/kb_driver/scram"
)

// INFO
const GOKB_Version_V008R006C002B0105 = iota
const GOKB_CompileTime_20210105_153951 = iota
const GOKB_CommitID_e83922c = iota
const GOKB_CompilerVersion_go1_11_5_linux_mips64le = iota
...
// how kb creates network connections.
...
func (cn *conn) handleKbpass(o values) {
	// if a password was supplied, do not process .kbpass
	...
	if filename == "" {
		...
		// XXX %APPDATA%\kingbase\kbpass.conf
	...
	mode := fileinfo.Mode()
	if mode&(0x77) != 0 {
		// XXX should warn about incorrect .kbpass permissions as psql does
		return
	}
	...
	db := o["dbname"]
	username := o["user"]
	// From: https://github.com/tg/kbpass/blob/master/reader.go
...
// scanner implements a tokenizer for libkci-style option strings.
type scanner struct {
	s []rune
	i int
}
...
//
// The parsing code is based on conninfo_parse from libkci's fe-connect.c
func parseOpts(name string, o values) error {
	...
	for {
		...
		if r, ok = s.SkipSpaces(); !ok {
			// If we reach the end here, the last value is just an empty string as per libkci.
			o[string(keyRunes)] = ""
			break
		}
...
// that Kingbase provides ('E') will be prepended to the string.
func QuoteLiteral(literal string) string {
	// This follows the Kingbase internal algorithm for handling quoted literals
	// from libkci, which can be found in the "PQEscapeStringInternal" function,
	// which is found in the libkci/fe-exec.c source file:
	// https://git.kingbase.org/gitweb/?p=kingbase.git;a=blob;f=src/interfaces/libkci/fe-exec.c
	//
	// substitute any single-quotes (') with two single-quotes ('')
	literal = strings.Replace(literal, `'`, `''`, -1)
	// determine if the string has any backslashes (\) in it.
	// if it does, replace any backslashes (\) with two backslashes (\\)
	// then, we need to wrap the entire string with a Kingbase
...
// parseEnviron tries to mimic some of libkci's environment handling
//
...
func parseEnviron(env []string) (out map[string]string) {
	...
	for _, v := range env {
		...
		// Kingbase 9.1 manual. Unsupported but well-defined
		// keys cause a panic; these should be unset prior to
		// execution. Options which kb expects to be set to a
		...
		case "KBPASSWORD":
			accrue("password")
		case "KBSERVICE", "KBSERVICEFILE", "KBREALM":
			unsupported()
		case "KBOPTIONS":
			accrue("options")
		case "KBAPPNAME":
		...
		case "KBSSLROOTCERT":
			accrue("sslrootcert")
		case "KBREQUIRESSL", "KBSSLCRL":
			unsupported()
		case "KBREQUIREPEER":
			unsupported()
		case "KBKRBSRVNAME", "KBGSSLIB":
			unsupported()
		case "KBCONNECT_TIMEOUT":
			accrue("connect_timeout")
		case "KBCLIENTENCODING":
...
func isUTF8(name string) bool {
	// Recognize all sorts of silly things as "UTF-8", like Kingbase does
